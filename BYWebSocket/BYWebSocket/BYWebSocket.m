...
BYSocketClosedInfo const BYSocketClosedCodeKey      = @"BYSocketClosedCodeKey";
BYSocketClosedInfo const BYSocketClosedReasonKey    = @"BYSocketClosedReasonKey";
BYSocketClosedInfo const BYSocketClosedCleanFlagKey = @"BYSocketClosedCleanFlagKey";

typedef NS_ENUM(NSInteger, BYSocketOpenType){
    BYSocketOpenByUrlString,
    BYSocketOpenByUrl,
    BYSocketOpenByRequest,
};


@interface BYWebSocket ()<SRWebSocketDelegate>

@property (nonatomic, strong, readwrite) SRWebSocket *webSocket;

@property (nonatomic, copy, readwrite) BYSocketDidConnectBlock connectSuccessBlock;
@property (nonatomic, copy, readwrite) BYSocketDidFailBlock connectFailBlock;
@property (nonatomic, copy, readwrite) BYSocketDidCloseBlock closeBlock;
@property (nonatomic, assign, readwrite) BOOL connecting;

@property (nonatomic, assign) BYSocketOpenType openType;
...
- (void)byopenWithUrlString:(NSString *)urlStr connect:(BYSocketDidConnectBlock)connectBlock failure:(BYSocketDidFailBlock)failureBlock {
    NSAssert(urlStr.length>0, @"Url String不能为空!");
    self.openUrlString = urlStr;
    self.openType = BYSocketOpenByUrlString;
    ...
    [self byopenWithRequest:request connect:connectBlock failure:failureBlock];
}

- (void)byopenWithUrl:(NSURL *)url connect:(BYSocketDidConnectBlock)connectBlock failure:(BYSocketDidFailBlock)failureBlock {
    NSAssert(url != nil, @"Url不能为空!");
    self.openUrl = url;
    self.openType = BYSocketOpenByUrl;
...
- (void)byopenWithRequest:(NSURLRequest *)request connect:(BYSocketDidConnectBlock)connectBlock failure:(BYSocketDidFailBlock)failureBlock {
    NSAssert(request != nil , @"Request参数不能为空!");
    self.openRequest = request;
    self.openType = BYSocketOpenByRequest;
...
- (void)bycloseSocketWithBlock:(BYSocketDidCloseBlock)closeBlock {
    self.closeBlock = closeBlock;
    [self p_closeSocketWithCode:BYSocketCloseNormalCode reason:@"Socket close normal!"];
}

- (void)bymanualStartHeartBeatAfterDelay:(NSTimeInterval)delay {
...
- (void)bysend:(NSDictionary *)dicData receive:(BYSocketDidReceiveBlock)receiveBlock failure:(BYSocketDidFailBlock)failureBlock {
...
- (void)p_send:(id)data withId:(NSString *)idStr {
    ...
    if (self.webSocket != nil) {
        switch (self.socketReadyState) {
            ...
            case SR_CLOSING: {
                if (self.closeBlock) {
                    self.closeBlock(BYSocketCloseErrorCode, @"Unkonw error cause socket close!", 0);
                }
                [[NSNotificationCenter defaultCenter] postNotificationName:kSocketCloseNotification object:self userInfo:@{@"code":@(BYSocketCloseErrorCode),@"error":@"Unkonw error cause socket close!"}];
            }
                break;
            case SR_CONNECTING: {
        ...
        [timer invalidate];
        timer = nil;
        BYSocketDidFailBlock failBlock = [sendRequestDic objectForKey:kReceiveFailBlockKey];
        if (failBlock) {
            failBlock([NSError errorWithDomain:_errorDomain code:BYSocketEmptyCode userInfo:@{NSLocalizedDescriptionKey: @"Send message fail BY socket was clean up"}]);
        }
...
- (void)p_reconnectSocket {
    ...
    if (_tryReconnectTimes >= _maxReconnectTimes) {
        //重连失败
        [self p_closeSocketWithCode:BYSocketCloseReconnectFailCode reason:@"Reconnect socket fail"];
        [[NSNotificationCenter defaultCenter] postNotificationName:kSocketReconnectFailNotification object:self userInfo:@{@"code":@(BYSocketCloseReconnectFailCode),@"reason":@"Reconnect socket fail"}];
        self.connecting = NO;
        return;
    }
    
    switch (self.openType) {
        case BYSocketOpenByUrlString: {
            [self byopenWithUrlString:self.openUrlString connect:self.connectSuccessBlock failure:self.connectFailBlock];
        }
            break;
        case BYSocketOpenByUrl: {
            [self byopenWithUrl:self.openUrl connect:self.connectSuccessBlock failure:self.connectFailBlock];
        }
            break;
        case BYSocketOpenByRequest: {
            [self byopenWithRequest:self.openRequest connect:self.connectSuccessBlock failure:self.connectFailBlock];
        }
            break;
        default:
            break;
...
-(void)p_checkTimeoutMsg:(NSString *)identificationID{
    ...
    if ([self.allSendRequestDic.allKeys containsObject:identificationID]) {
        ...
        BYSocketDidFailBlock failure = callBackDic[kReceiveFailBlockKey];
        if (failure) {
            failure([NSError errorWithDomain:_errorDomain code:BYSocketTimeoutCode userInfo:@{NSLocalizedDescriptionKey: @"Timeout Connecting to Server"}]);
        }
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    if (![parseDataDic.allKeys containsObject:kIdentificationIDKey]) {
        NSLog(@"%@",[NSError errorWithDomain:_errorDomain code:BYSocketReceiveWrongFormatDataCode userInfo:@{NSLocalizedDescriptionKey: @"Socket receive wrong format data"}]);
        return;
    }
    
    identificationID = parseDataDic[kIdentificationIDKey];
    if(![self.allSendRequestDic.allKeys containsObject:identificationID]) {
        NSLog(@"%@",[NSError errorWithDomain:_errorDomain code:BYSocketReceiveWrongIdentifyIdCode userInfo:@{NSLocalizedDescriptionKey: @"Received wrong identify id!"}]);
        return;
    }
    NSDictionary *sendRequestDic = self.allSendRequestDic[identificationID];
    
    if (webSocket == self.webSocket) {
        BYSocketDidReceiveBlock receiveBlock = sendRequestDic[kReceiveSuccessBlockKey];
        if (receiveBlock) {
            receiveBlock(parseDataDic, BYSocketReceiveTypeForMessage);
        }
        
        if([self.delegate respondsToSelector:@selector(BYWebSocket:didReceiveMessage:)]) {
            [self.delegate BYWebSocket:webSocket didReceiveMessage:parseDataDic];
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:kSocketReceiveMessageNotification object:parseDataDic];
    } else {
        BYSocketDidFailBlock failBlock = sendRequestDic[kReceiveFailBlockKey];
        if (failBlock) {
            failBlock([NSError errorWithDomain:_errorDomain code:BYSocketChangedCode userInfo:@{NSLocalizedDescriptionKey: @"Socket did changed!"}]);
        }
    }
    NSTimer *timer = [sendRequestDic objectForKey:kSendTimerKey];
    [timer invalidate];
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    if ([self.delegate respondsToSelector:@selector(BYWebSocketDidOpen:)]) {
        [self.delegate BYWebSocketDidOpen:webSocket];
    }
...
- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error {
    ...
    if ([self.delegate respondsToSelector:@selector(BYWebSocket:didFailWithError:)]) {
        [self.delegate BYWebSocket:webSocket didFailWithError:error];
    }
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    if ([self.delegate respondsToSelector:@selector(BYWebSocket:didCloseWithCode:reason:wasClean:)]) {
        [self.delegate BYWebSocket:webSocket didCloseWithCode:code reason:reason wasClean:wasClean];
    }
    switch (code) {
        case BYSocketCloseToReconnectCode: {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(_tryReconnectTimes * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self p_destoryHeartBeat];
                [self p_reconnectSocket];
            });
        ...
        default: {
            ...
            //            @{BYSocketFailCodeKey: formatString(@"%zd", code), BYSocketFailReasonKey: reason, BYSocketFailCleanFlagKey: formatString(@"%d", wasClean)}
            [[NSNotificationCenter defaultCenter] postNotificationName:kSocketCloseNotification object:self userInfo:@{BYSocketClosedCodeKey: [NSNumber numberWithInteger:code], BYSocketClosedReasonKey: reason ?: @"", BYSocketClosedCleanFlagKey: [NSNumber numberWithBool:wasClean]}];
        }
            break;
    }
...
- (void)webSocket:(SRWebSocket *)webSocket didReceivePong:(NSData *)pongPayload {
    self.tryPingTimes = 0;
    if ([self.delegate respondsToSelector:@selector(BYWebSocket:didReceivePong:)]) {
        [self.delegate BYWebSocket:webSocket didReceivePong:pongPayload];
    }
}

// Return YES to convert messages sent as Text to an NSString. Return NO to skip NSData -> NSString conversion for Text messages. Defaults to YES.
- (BOOL)webSocketShouldConvertTextFrameToString:(SRWebSocket *)webSocket {
    if ([self.delegate respondsToSelector:@selector(BYWebSocketShouldConvertTextFrameToString:)]) {
        return [self.delegate BYWebSocketShouldConvertTextFrameToString:webSocket];
    }
    return YES;
}
